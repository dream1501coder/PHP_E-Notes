<?php
  ...
  if($_SERVER['REQUEST_METHOD']=='POST'){
      ...
      if($result){
        ...
        $sql_user="select * from login_details where `username`='$current'";
  ...
  $sql="INSERT INTO `notes` (`title`, `desc`,`user_id`) VALUES ('$title', '$desc','$user_id');";
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="notes.php?update=true" method="post">
...
if($delete){
    // echo"<script>window.location=`/PHP_E-Notes/notes.php?delete=${sno}`;</script>";
    echo '<svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
    ...
    ?>
            ...
            <div class="container">
                <?php
                // echo $current=$_SESSION['username'];
                $user=$_SESSION['username'];
                
                $sql_user="select * from login_details where `username`='$user'";
                $res_user=mysqli_query($conn,$sql_user);
                $row=mysqli_fetch_assoc($res_user);
                $userid=$row['id'];
                // echo $userid;
                ...
                Array.from(deletes).forEach((element) => {
                    element.addEventListener("click", (e) => {
                        ...
                        if (confirm("Are you sure you want to delete this note ?? ")) {
                            console.log("yes");
                            window.location=`/PHP_E-Notes/notes.php?delete=${sno}`;
                        }
                        else{
                            console.log("no");
